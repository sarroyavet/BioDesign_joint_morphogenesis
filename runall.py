  ...
  with open(filename, 'r') as csvfile: 
    ...
    for item in case:
      ...
      "AD_param":{
                    ...
                    "Normal_vm_lim": (float(item['vmlimit']) if 'vmlimit' in item.keys() else 0.2829),
                    "lamda_mall_zone": (float(item['lambda']) if 'lambda' in item.keys() else 2.1),
                    "alphaini": (float(item['alphaini']) if 'alphaini' in item.keys() else 0.0560),
                    "gamma": (float(item['gamma']) if 'gamma' in item.keys() else 0.2),
                    "capsulparam": (float(item['capsulparam']) if 'capsulparam' in item.keys() else 0.017),
                    "theta": (float(item['theta']) if 'theta' in item.keys() else 8028.0646),
